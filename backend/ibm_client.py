...
DEPLOYMENT_URL = (
    "https://private.eu-de.ml.cloud.ibm.com/ml/v4/deployments/2e248a30-ee3c-41eb-9025-7ec17fc4c731/predictions?version=2021-05-01"
)
...
def score_prompt(prompt: str, api_key: Optional[str] = None) -> Dict[str, Any]:
    """Send the prompt to the deployment and return the raw JSON response.

    Uses `prompt` as the only field in the input payload. If your deployment
    expects other fields, adapt the `payload_scoring` structure accordingly.
    """
    if api_key is None:
